...
function setGoogleMap(query = state.mapQuery, zoom = state.mapZoom) { state.mapQuery = query; state.mapZoom = Math.max(1, Math.min(10, zoom)); const encodedQuery = encodeURIComponent(state.mapQuery); elements.googleMap.src = googleMapsKey ? `https://www.google.com/maps/embed/v1/place?key=${encodeURIComponent(googleMapsKey)}&q=${encodedQuery}&zoom=${state.mapZoom}` : `https://maps.google.com/maps?q=${encodedQuery}&z=${state.mapZoom}&output=embed`; }
...
function renderRound() {
  ...
  state.selected = null; state.locked = false; state.timeLeft = 90; state.hintUsed = false; state.timerPaused = false; state.timerDeadline = performance.now() + 90000; setGoogleMap('0,0', 1);
...
function revealLocation() { const round = currentRound(); elements.mapLocation.textContent = `${round.city}, ${round.country}`; elements.mapCoordinates.textContent = round.coordinates; setGoogleMap(`${round.city}, ${round.country}`, 8); elements.mapPin.classList.remove('is-hidden'); elements.routeCurrentNode.classList.remove('is-hidden'); renderRoute(); }
...
function setZoom(nextZoom) { state.zoom = Math.max(-1, Math.min(1, nextZoom)); state.mapZoom = state.zoom > 0 ? 8 : state.zoom < 0 ? 1 : state.locked ? 8 : 1; elements.mapStage.classList.toggle('zoomed-in', state.zoom > 0); elements.mapStage.classList.toggle('zoomed-out', state.zoom < 0); if (elements.mapStage.classList.contains('google-maps-mode')) setGoogleMap(state.mapQuery, state.mapZoom); setMapToast(`Map zoom ${state.zoom === 0 ? 'reset' : state.zoom > 0 ? 'in' : 'out'}.`); }
function recenterMap() { state.zoom = 0; state.mapZoom = state.locked ? 8 : 1; state.mapOffset = { x: 0, y: 0 }; elements.worldMap.style.translate = '0px 0px'; elements.mapStage.classList.remove('zoomed-in', 'zoomed-out'); elements.mapYou.setAttribute('transform', 'translate(0 0)'); setGoogleMap(state.mapQuery, state.mapZoom); setMapToast('Map view reset to the current stop.'); }
...
elements.lockButton.addEventListener('click', () => submitGuess()); elements.nextButton.addEventListener('click', nextRound); elements.hintButton.addEventListener('click', useHint); elements.layersButton.addEventListener('click', () => { const active = elements.mapStage.classList.toggle('google-maps-mode'); elements.layersButton.classList.toggle('is-active', active); elements.layersButton.setAttribute('aria-pressed', String(active)); if (active) setGoogleMap(state.mapQuery, state.mapZoom); setMapToast(active ? 'Google Maps layer enabled.' : 'Offline atlas layer enabled.'); }); elements.locateButton.addEventListener('click', recenterMap); elements.zoomIn.addEventListener('click', () => setZoom(state.zoom + 1)); elements.zoomOut.addEventListener('click', () => setZoom(state.zoom - 1));
